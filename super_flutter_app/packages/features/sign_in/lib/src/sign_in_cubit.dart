...
class SignInCubit extends Cubit<SignInState> {
  ...
  void onEmailChanged(String newValue) {
    final previousScreen = state;
    final previousEmailState = previousScreen.email;
    final shouldValidate = previousEmailState.invalid;
    final newEmailState = shouldValidate
        ? Email.validated(newValue)
        : Email.unvalidate(newValue);

    final newScreenState = state.copyWith(email: newEmailState);
    emit(newScreenState);
  }

  void onEmailInFocused() {
    final previousScreenState = state;
    final previousEmailState = previousScreenState.email;
    final previousEmaileValue = previousEmailState.value;

    final newEmailState = Email.validated(previousEmaileValue);
  ...
  void onPasswordChanged(String newValue) {
    ...
    final newPassworState = shouldValidate
        ? Password.validate(newValue)
        : Password.unvalidated(newValue);

    final newScreenState = state.copyWith(password: newPassworState);
    emit(newScreenState);
  }

  void onPasswordUnfoucesd() {
    final previousScreenState = state;
    final previousPasswordState = previousScreenState.password;
    final previousPasswordValue = previousPasswordState.value;

    final newPasswordState = Password.validate(previousPasswordValue);
  ...
  void onSubmit() async {
    ...
    final password = Password.validate(state.password.value);
