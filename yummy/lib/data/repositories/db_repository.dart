import 'package:basic_widgets/data/database/recipe_db.dart';
...
class DBRepository extends Notifier<CurrentRecipeData> implements Repository {
  ...
  Future<void> deleteIngredient(Ingredient ingredient) {
    if (ingredient.id != null) {
      final udpatedList = [...state.curentIngredients];
      udpatedList.remove(ingredient);
      state = state.copyWith(curentIngredients: udpatedList);
  ...
  Future<List<Ingredient>> findAllIngredients() {
    return _ingredientDao.findAllIngredients().then((
  ...
  Future<List<Recipe>> findAllRecipes() {
    return _recipeDao.findAppRecipes().then((
      List<DbRecipeData> dbRecippes,
    ) async {
      final recipes = <Recipe>[];
      for (final dbRecipe in dbRecippes) {
